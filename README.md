...
1. **Scene generation** (`grasp2scene_mocap.py`): reads object 6D poses from GraspNet annotations and generates a MuJoCo XML scene with dynamic objects and a kinematic Panda hand.

2. **Grasp publishing** (`utils/poses.py`): loads grasp labels, filters by friction coefficient ($\mu \leq \text{thresh}$) and collision, then builds a `GraspGroup`.

3. **Simulation** (`grasp_sim_mocap.py`): for each grasp pose, transforms it from camera frame to world frame and then executes an approach → close → lift sequence. Success is determined by checking $\Delta z$ of target objects post-lift.
